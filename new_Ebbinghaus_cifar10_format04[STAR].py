...
SAVE_PATH = "Z:/STUDY/checkpoint/02"
...
def Train(step, model, BATCH_SIZE, optimizer, scheduler, cost, use_gpu):
    ...
    transform = transforms.Compose([transforms.CenterCrop(32),  # Crop from the middle
                                    ...
                                    transforms.Normalize([0.5, 0.5, 0.5],
                                                         [0.5, 0.5, 0.5])])  # Let Tensor from [0, 1] to [-1, 1]
    ...
    for data in data_loader_image:
        ...
        logger.info(
            "Epoch: {} Step: {} Batchsize: {} Ave_Loss: {:.5f} Ave_Acc: {:.2f} % Step_Loss: {:.5f} Step_Acc: {:.2f} % All_data: {} Train_Time: {:.0f} h {:.0f} m {:.2f} s Used: {} % {:.0f}/{}".format(
                epoch + 1,
                All_step,
                BATCH_SIZE,
    ...
    for step in range(max_list_limit + 1):
        ...
        if used_data % 500000 == 0:
            if WHETHER_SAVE_MODEL:
                torch.save(model.state_dict(), (SAVE_PATH + "model_batch" + str((All_step+1) * BATCH_SIZE) + ".pkl"))